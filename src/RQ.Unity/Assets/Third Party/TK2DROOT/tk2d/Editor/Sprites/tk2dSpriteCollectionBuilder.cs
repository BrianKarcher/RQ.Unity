    ...
    static void UpdateVertexCache(tk2dSpriteCollection gen, float scale, tk2dEditor.Atlas.Data[] packers, tk2dSpriteCollectionData coll, List<SpriteLut> spriteLuts)
    {
        for (int i = 0; i < sourceTextures.Length; ++i)
        {
            ...
            else
            {
                ...
                if (gen.textureParams[i].UseCustomUV)
                {
                    coll.spriteDefinitions[i].uvs = gen.textureParams[i].CustomUV;
                    for (int l = 0; l < coll.spriteDefinitions[i].uvs.Length; l++)
                    {
                        coll.spriteDefinitions[i].uvs[l] = coll.spriteDefinitions[i].uvs[l] + gen.textureParams[i].UvOffset;
                    }
                }
