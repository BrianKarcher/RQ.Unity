...
namespace PixelCrushers.DialogueSystem.NGUISupport
{
    ...
    /// </summary>
    [AddComponentMenu("Pixel Crushers/Dialogue System/Third Party/NGUI/NGUI Quest Tracker (on Dialogue Manager)")]
    public class NGUIQuestTracker : MonoBehaviour
    {
        ...
        private UITable containerTable = null;
        ...
        public void Start()
        {
            if (container == null)
            {
                Debug.LogWarning(string.Format("{0}: {1} Container is unassigned", new object[] { DialogueDebug.Prefix, name }), this);
            }
            else
            {
                containerTable = container.GetComponent<UITable>();
            }
        ...
        public void UpdateTracker()
        {
            ...
            if (containerTable != null) containerTable.repositionNow = true;
        }
