...
namespace PixelCrushers.DialogueSystem.NGUISupport
{
    ...
    /// template instead.
    /// </summary>
    [AddComponentMenu("Pixel Crushers/Dialogue System/Third Party/NGUI/NGUI Bark UI")]
    public class NGUIBarkUI : MonoBehaviour, IBarkUI
    {
        ...
        public void Bark(Subtitle subtitle)
        {
            ...
            barker = subtitle.speakerInfo.transform;
            if (label != null)
            {
                if (includeName)
                {
        ...
        {
            SendMessage("OnBarkEnd", barker, SendMessageOptions.DontRequireReceiver); // Temp. fix because OnBarkEnd isn't being sent.
            secondsLeft = 0;
            Hide();
        }
